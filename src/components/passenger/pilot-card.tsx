...
import { ArrowRight, MessageSquare, Wallet, MapPin } from 'lucide-react';
...
export default function PilotCard({ ride }: PilotCardProps) {
  ...
  const handleRequestSent = () => {
    toast({
      title: 'Request Sent!',
      description: 'Your ride request has been sent to the pilot.',
    });
    setOpen(false);
  };
    ...
    <Card className="flex flex-col overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1">
      <CardHeader className="flex-row gap-4 items-center">
        <Avatar className="h-12 w-12">
          <AvatarFallback>
            {ride.pilotId?.charAt(0)?.toUpperCase() ?? 'P'}
          </AvatarFallback>
        </Avatar>
        ...
        <div>
          ...
          <CardDescription>
            ID: {ride.pilotId?.slice(0, 8) ?? 'Pending'}
          </CardDescription>
        </div>
      </CardHeader>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Review your ride details before sending request.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="offered-fare">
                  Your Fare Offer (Optional)
                </Label>
                <div className="relative">
                  ...
                  <Input
                    id="offered-fare"
                    type="number"
                    placeholder={String(ride.fareEstimate ?? '')}
                    className="pl-6"
                  />
                </div>
                <p className="text-xs text-muted-foreground">
                  Pilot may accept or reject your offer.
                </p>
              </div>
            </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleRequestSent}>
                Send Request
              </Button>
            </DialogFooter>
          </DialogContent>
